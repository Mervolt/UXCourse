...
export default class FiszkiListView extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      currFiszki: this.props.navigation.state.params.fiszki.sort(
        this.compareFiszki
      ),
      searchField: "",
      ...
      <View>
        ...
        >
          <Button
            ...
            icon={
              <Icon name={"add"} reverse size={20} color={"rgb(255,94,0)"} />
            }
  ...
  searchFiszki(text) {
    const newData = this.state.allFiszki
      .filter(function(fiszka) {
        if (text === "") return true;
